...
#     if len(set(list(V1)[:-2]).intersection(set(list(V2)[:-2]))) > 0:
#         return 0
...
def p2(S, iter=26):
    valves = set([i for i, j in S.items() if j['rate'] > 0])
    nr_elm = len(valves) // 2 if len(valves) % 2 == 0 else len(valves) // 2 + 1
    res = []
    comb = set(combinations(valves, nr_elm))
    for i in comb:
        S_copy = deepcopy(S)
        set_to_zero = set(i) ^ valves
        for j in set_to_zero:
            S_copy[j]['rate'] = 0
        res.append(p11(S_copy, iter))

    return sum(sorted(res)[-2:])
